...
# Define the three head pooling methods to iterate over.
# methods=("Qproj" "Qavgproj" "Qavg" "Qorig")

headpooling_type=${HEADPOOLING_TYPE:-"Qproj"}
gate_type=${GATE_TYPE:-"Kmaxminavg"}  

threshold=${TRAINING_THRESHOLD:-0}   # 0 for no threshold             
gate_hidden_size=${SEERATTN_GATE_HIDDEN_SIZE:-128}     
blocksize=${SEERATTN_BLOCK_SIZE:-64}
ignore_last_block=${IGNORE_LAST_BLOCK:-true}         
...
output_dir=${OUTPUT_DIR:-"/mnt/output/SeerDecode/post-training-baseline"}
steps=${STEPS:-800}
bs=${BS:-16}
...
torchrun --nproc_per_node=$gpus --master_port=10003 distillation_new.py  \
