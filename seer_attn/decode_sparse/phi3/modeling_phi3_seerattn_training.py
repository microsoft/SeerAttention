...
from seer_attn.phi3.configuration_phi3_seerattn import SeerAttnPhi3Config
...
from seer_attn.modules.common import apply_rotary_pos_emb, get_indice, repeat_kv, repeat_kv_varlen, RMSNorm
...
class SeerAttnPhi3Attention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        attn_output, _, mask1d_ground_truth = attn_pooling(
            q,
            k,
            v,
            cu_seqlens,
        ...
        bsz = mask1d_ground_truth.shape[0]
...
class SeerAttnPhi3DecoderLayer(nn.Module):
    def __init__(self, config: SeerAttnPhi3Config, layer_idx: int):
        super().__init__()
...
class SeerAttnPhi3PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            module.weight.data.normal_(mean=0.0, std=std)
        elif isinstance(module, Phi3RMSNorm) or isinstance(module, RMSNorm):
            module.weight.data.fill_(1.0)


class SeerAttnPhi3Model(SeerAttnPhi3PreTrainedModel):
    """
    Transformer decoder consisting of *config.num_hidden_layers* layers. Each layer is a [`SeerAttnPhi3DecoderLayer`]

    Args:
        config: SeerAttnPhi3Config
    """
    ...
    def __init__(self, config: SeerAttnPhi3Config):
        ...
        self.layers = nn.ModuleList(
            [SeerAttnPhi3DecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)]
        )
        ...
        self.rotary_emb_gate_q = Phi3RotaryEmbedding(config=block_config)
...
class SeerAttnPhi3ForCausalLM(SeerAttnPhi3PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        super().__init__(config)
        self.model = SeerAttnPhi3Model(config)
        self.vocab_size = config.vocab_size
