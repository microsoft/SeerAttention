...
class Qwen3Model(Qwen3PreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            ...
            position_ids = cache_position.unsqueeze(0) ## shape [1, seqlen]
...
class SeerDecodingQwen3ForCausalLM(Qwen3PreTrainedModel, GenerationMixin):
    ...
    def static_cache_generate(
        ...
        **model_kwargs,
        ...
        """
        Static preallocates the cache and uses it for generation.
        """
        ...
        for step in range(max_length - generated.shape[1]):
            ...
            if isinstance(generation_config.eos_token_id, list):
                is_eos = torch.isin(next_tokens.squeeze(-1), eos_token_ids)
            else:
                is_eos = (next_tokens.squeeze(-1) == eos_token_id)
            ...
            if attention_mask is not None:
                attention_mask = torch.cat([
                    attention_mask, 
                    torch.zeros((batch_size, 1), dtype=attention_mask.dtype, device=device)
                ], dim=-1)
                attention_mask[batch_indices, cache_seqlens] = 1  # Set the new position to 1
