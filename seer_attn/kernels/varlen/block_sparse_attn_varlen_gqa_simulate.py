...
# @triton.autotune(
#     configs=[
#         triton.Config({'BLOCK_M': BM,}, num_warps=num_warps) 
#         for BM in [128] 
#         for num_warps in [2, 4, 8, 16] #, 2, 4, 8, 16
#     ],
#     key=['q_k_ratio', 'BLOCK_N', ],
# )
@triton.jit
def _fwd_kernel_varlen(
    ...
    block_mask_ptr += off_z * stride_bmask_z + off_h_for_kv * stride_bmask_h + offs_m * stride_bmask_m
    ...
    for k_block_col_idx in range(k_block_start, k_block_end):
        mask_val = tl.load(block_mask_ptr + k_block_col_idx * stride_bmask_n, mask=offs_m < seqlen_k)
        ...
        qk = tl.dot(q, k)
        qk *= sm_scale
        qk += tl.where(mask_val[:, None], 0, -1.0e6)
    ...
    tl.store(l_ptrs, m_i, mask=offs_m < seqlen_q) 
...
def blocksparse_flash_attn_varlen_fwd(
    ...
    if is_hip():
        extra_kern_args = {"waves_per_eu":2, "allow_flush_denorm": True}
    else:
        extra_kern_args = {}
...
#         triton.Config({'BLOCK_M': BM,}, num_warps=num_warps) 
#         for BM in [32, 64, 128] 
#         for num_warps in [1, 2, 4, 8, 16] #, 2, 4, 8, 16
#     ],
#     key=['H', 'BLOCK_N',],
# )
@triton.jit
def _bwd_dkdv(
    ...
    num_block = tl.cdiv(N_CTX, BLOCK_M)
    start_l = start_n * BLOCK_N // BLOCK_M
    end_l = num_block
    ...
    for row_idx in range(start_l, end_l):
        ...
        mask_m = offs_m < N_CTX
        mask_val = tl.load(block_mask_ptr + offs_m * stride_bmask_m, mask=mask_m)    
        ...
        qk += tl.where(offs_m[:, None] >= (offs_n[None, :]), 0, -1.0e6)
        l_i = tl.load(l_offset + offs_m * stride_lm, mask=mask_m)
        p = tl.exp(qk * sm_scale - l_i[:, None])
        
        p_mask = mask_m[:, None] & mask_n[None, :] & mask_val[:, None]
...
def _bwd_dq(
    ...
    block_mask_ptr += off_z * stride_bmask_z + off_h * stride_bmask_h + offs_m * stride_bmask_m

    start_l = 0
    end_l = tl.cdiv((start_m + 1) * BLOCK_M, BLOCK_N)

    for col_idx in range(start_l, end_l):
        mask_val = tl.load(block_mask_ptr + col_idx * stride_bmask_n, mask=mask_m)
        ...
        qk = tl.zeros([BLOCK_M, BLOCK_N], dtype=tl.float32)
        qk += tl.dot(q, tl.trans(k))
        # mask_val = (1 - mask_val).to(tl.float32) * (-1e9)
...
def blocksparse_flash_attn_varlen_bwd(
    ...
    with torch.cuda.device(q.device.index): 
        ...
        grid = lambda args: (triton.cdiv(max_seqlen, args["BLOCK_M"]), batch_headsize)
...
block_sparse_attn_varlen_func_perq = _block_sparse_attn_varlen.apply