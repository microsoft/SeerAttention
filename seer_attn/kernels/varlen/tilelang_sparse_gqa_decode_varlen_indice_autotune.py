...
def get_configs():
    ...
    block_H_candidates = [32, 64, 128]
    num_stages_candidates = [1, 2, 3, 4, 7]
    threads_candidates = [64, 128, 256]
    configs = [
        {"block_H": block_H, "num_stages": num_stages, "threads": threads}
...
def get_best_config(batch, heads, heads_kv, dim, dim_v, block_size):
    """Use AutoTuner to find the best configuration based on performance."""
    def kernel_wrapper(block_H, num_stages, threads):
...
class SparseFlashAttn(torch.nn.Module):
    ...
    def __init__(self, batch, heads, heads_kv, dim, dim_v, block_size):
        ...
        best_config = get_best_config(batch, heads, heads_kv, dim, dim_v, block_size)
