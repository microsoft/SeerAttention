...
from seer_attn.qwen_decode_sparse.attn_gate import ATTNGATE_CLASSES_TRAIN
from seer_attn.kernels.attn_pooling_kernel_with_perq import attn_with_pooling
...
def compute_topk_overlap(mask1d_ground_truth, mask1d_gate_prediction):
    
    B, H, S, _ = mask1d_ground_truth.shape
    k = max(1, int(round(S * 0.1)))

    # selected_rows = [-2, -1]

    mask1d_ground_truth = mask1d_ground_truth[:,:,-1]
    mask1d_gate_prediction = mask1d_gate_prediction[:,:,-1]
    
    indices_gt = torch.topk(mask1d_ground_truth, k=k, dim=-1, sorted=False).indices
    indices_pred = torch.topk(mask1d_gate_prediction, k=k, dim=-1, sorted=False).indices
    
    mask_gt = torch.zeros_like(mask1d_ground_truth, dtype=torch.bool)
    mask_gt.scatter_(-1, indices_gt, True)
    
    mask_pred = torch.zeros_like(mask1d_gate_prediction, dtype=torch.bool)
    mask_pred.scatter_(-1, indices_pred, True)

    # overlap_ratios = []
    # for i in range(H):
    #     overlap_ratio = (mask_gt[:,i,:] & mask_pred[:,i,:]).sum().item() / (mask_gt[:,i,:]).sum().item() * 100
    #     overlap_ratios.append(overlap_ratio)

    overlap_ratio = (mask_gt & mask_pred).sum().item() / (mask_gt).sum().item() * 100
...
class SeerAttnQwen2Attention(nn.Module):
    ...
    def __init__(self, config: SeerAttnQwen2Config, layer_idx: int):
        ...
        self.attn_gate = ATTNGATE_CLASSES_TRAIN[config.seerattn_gate_type](
        ...
        self.train_both = os.environ.get("TRAIN_BOTH", False)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        # seq_len = (hidden_states.shape[2] // 64) * 64
        ...
        if self.train_both:
            mask2d_gate_prediction = self.attn_gate(query_states, key_states, block_attention_mask, block_position_embeddings, do_q_pooling=True, use_softmax=False)
        ...
        q_block_slice = query_states[:, :, (block_size - 1)::block_size]
        mask1d_gate_prediction = self.attn_gate(q_block_slice, k_unroped, block_attention_mask, block_position_embeddings, do_q_pooling=False, use_softmax=False) ## only for training now
        ...
        ### removed the sliding window part
        ...
        # seq_len = (query_states.shape[2] // 64) * 64
        ...
        if self.training:
            ...
            attn_output, mask2d_ground_truth, mask1d_ground_truth = attn_with_pooling(
                query_states,
                key_states,
                value_states,
                True, 
            ...
            mask1d_ground_truth = mask1d_ground_truth[:, :, (block_size - 1)::block_size].float()
            # ignore the last block because of padding 64
            mask1d_ground_truth = mask1d_ground_truth[:, :, :-1]
            mask1d_gate_prediction = mask1d_gate_prediction[:, :, :-1, :-1]

            # mask1d_gate_prediction_softmax = F.softmax(mask1d_gate_prediction, dim=-1)
            mask1d_gate_prediction = F.log_softmax(mask1d_gate_prediction.float(), dim=-1)
            mask_loss_1d = self.mask_loss_func(mask1d_gate_prediction, mask1d_ground_truth)

            # recall rate
            # overlap = compute_topk_overlap(mask1d_ground_truth, mask1d_gate_prediction)
            # if self.layer_idx == 0:
            #     print(f"overlap: {overlap:.2f}   len: {query_states.shape[2]}")
            # with open("./overlap_log.txt", "a") as f:
            #     f.write(f"{overlap:.2f}% {query_states.shape[2]}\n")

            if self.train_both:
                mask2d_ground_truth = mask2d_ground_truth.float()
                mask2d_gate_prediction = F.log_softmax(mask2d_gate_prediction.float(), dim=-1)
                mask_loss_2d = self.mask_loss_func(mask2d_gate_prediction, mask2d_ground_truth)
            else:
                mask_loss_2d = 0.0
            mask_loss = mask_loss_1d + mask_loss_2d
...
class SeerAttnQwen2Model(SeerAttnQwen2PreTrainedModel):
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        # Added for seerattn 
        block_position_ids = position_ids[:, (self.config.seerattn_gate_block_size - 1)::self.config.seerattn_gate_block_size] ## downsampled position ids
