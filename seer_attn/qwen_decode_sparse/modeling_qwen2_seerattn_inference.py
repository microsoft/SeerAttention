...
from seer_attn.qwen_decode_sparse.attn_gate import ATTNGATE_CLASSES
...
from seer_attn.kernels.block_sparse_decode import block_sparse_decode,block_sparse_decode_pt
from seer_attn.qwen_decode_sparse.pooling_cache import PoolingCache
...
class SeerAttnQwen2Attention(nn.Module):
    ...
    def forward(
        ...
        pooling_cache: Optional[Cache] = None,
        ...
        **kwargs,
        ...
        if not self.training:
            # cache for the last 64 tokens
            k_remainder = pooling_cache.update_k_remainder_cache(k_unroped, self.layer_idx, q_len, self.config.seerattn_gate_block_size)
        ...
        mask1d_gate_prediction = self.attn_gate(query_states, k_unroped, k_remainder, self.layer_idx, pooling_cache, block_attention_mask, block_position_embeddings, use_softmax=use_softmax)    
        ...
        if q_len == 1 and self.config.use_decode_seerattn: ## decode inference
            # the last block is always true for decoding
            mask1d_gate_prediction = mask1d_gate_prediction.squeeze(2)
            if self.config.seerattn_sparsity_method == "nz_ratio":
                ...
                topk_nz_ratio = self.config.seerattn_nz_ratio
                topk = int(topk_nz_ratio * downsampled_len)
                topk = 1 if topk == 0 else topk
                
                topk_indices = torch.topk(mask1d_gate_prediction, k=topk, dim=-1, sorted=False).indices
                sparse_attn_mask = torch.zeros_like(mask1d_gate_prediction, dtype=torch.bool)
                sparse_attn_mask.scatter_(-1, topk_indices, True)
                sparse_attn_mask[:,:,-1] = True


            elif self.config.seerattn_sparsity_method == "threshold":
                sparse_attn_mask = mask1d_gate_prediction > self.config.seerattn_threshold
                sparse_attn_mask[:,:,-1] = True

                # log the sparsity percentage
                false_percentage = (1 - sparse_attn_mask.sum().item() / sparse_attn_mask.numel()) * 100
                with open("./percentage_replaced_log.txt", "a") as f:
                    f.write(f"{false_percentage:.2f}% {k_len}\n")

            else:
                raise NotImplementedError("The sparsity method is not implemented")


            attn_output = block_sparse_decode(
                query_states,
                key_states,
                value_states,
                sparse_attn_mask,
...
class SeerAttnQwen2ForCausalLMInf(SeerAttnQwen2PreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        ...
        self.pooling_cache = PoolingCache(self.num_layers, self.block_size)
    ...
    def forward(
        ...
        outputs = self.model(
            ...
            position_ids=position_ids,
            past_key_values=past_key_values,
            pooling_cache = self.pooling_cache,
