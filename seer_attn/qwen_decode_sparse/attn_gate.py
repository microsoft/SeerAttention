### Training and Inference AttnGate module for decode sparsity. It's currently unoptimized and can be slow. 
...
class AttnGate(nn.Module):
    ...
    def forward(self, 
        ...
        do_q_pooling = q.shape[2] != 1

        prefill_update_flag = False
        decode_update_flag = False


        if do_q_pooling:
            if q.shape[2] % self.block_size == 0:
                prefill_update_flag = True
            ...
            if k_remainder.shape[2] % self.block_size == 0:
                decode_update_flag = True
                k_remainder_pooled = [pool_func(k_remainder, kernel_size=[self.block_size, 1], stride=[self.block_size, 1], ceil_mode=True) for pool_func in self.k_pooling_funcs]
                k = torch.cat(k_remainder_pooled, dim=-1)
                k = self.mask_linear_k(k)
        ...
        attn = torch.matmul(q, k.transpose(-1, -2)) / torch.sqrt(torch.tensor(self.hidden_size))

        if use_softmax:
            attn = F.softmax(attn, dim=-1)
        return attn

class TrainingAttnGate(nn.Module):
    def __init__(self, block_size, in_channel_size, hidden_size, num_k_head, num_q_head, q_pooling_funcs, k_pooling_funcs):
        super(TrainingAttnGate, self).__init__()
        self.block_size = block_size
        self.in_channel = in_channel_size
        self.hidden_size = hidden_size
        self.num_k_head = num_k_head
    ...
    # q shape (batch_size, num_q_head, seq_length, channel_size)
    # k shape (batch_size, num_k_head, seq_length, channel_size)
    def forward(self, q, k, attention_mask=None, position_embeddings=None, do_q_pooling=True, use_softmax=False):  
...
def generate_combinations_train():
    new_classes = {}
    pool_types = ['max', 'min', 'avg']
    ...
    for q_comb in range(1, 4):
        for k_comb in range(1, 4):
            for q_pooling_comb in combinations(pool_types, q_comb):
                for k_pooling_comb in combinations(pool_types, k_comb):
                    class_name, new_class = _create_generic_attngate_class(TrainingAttnGate, '', q_pooling_comb, k_pooling_comb)
                    new_classes[class_name] = new_class
    return new_classes
...
ATTNGATE_CLASSES_TRAIN = generate_combinations_train()