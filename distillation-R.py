...
from seer_attn.llama.modeling_llama_seerattn_perq_training import LlamaForCausalLM
from seer_attn.qwen2.modeling_qwen2_seerattn_training import SeerAttnQwen2ForCausalLM
from seer_attn.qwen3.modeling_qwen3_seerattn_training import SeerAttnQwen3ForCausalLM
from seer_attn.phi3.modeling_phi3_seerattn_training import SeerAttnPhi3ForCausalLM
...
class ModelArguments:
    ...
    seerattn_training_threshold: Optional[float] = field(
        default=0.0,
        metadata={"help": "Training threshold, 0.0 means don't use threshold."},
    )
    seerattn_gate_hidden_size: Optional[int] = field(
        default=128,
        metadata={"help": "AttnGate hidden size."},
    )
    seerattn_ignore_last_block: Optional[bool] = field(
        default=False,
        metadata={"help": "ignore_last_block"},
    )
    ...
    seerattn_block_slice_mode: Optional[bool] = field(
        default=False,
        metadata={"help": "use block slice mode in attn gate training."},
    )
...
def train():
    ...
    if "llama" in model_args.model_name_or_path.lower():
    ...
    elif "qwen2" in model_args.model_name_or_path.lower() or "r1-distill-qwen" in model_args.model_name_or_path.lower():
        ...
        model_cls = SeerAttnPhi3ForCausalLM
    else:
        raise ValueError("Model not supported. Please use llama, phi3, qwen2 or qwen3 model.")
